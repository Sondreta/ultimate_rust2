...
fn main() {
    ...
    let transformed : Vec<_> = words.into_iter().filter( |w| !w.contains('h') ).map( |w| { w.to_uppercase() }  ).collect();
    println!("Transformed: {:?}", transformed);

    // Challenge:
    //
    // - Rewrite the code in #2 as a for loop
    let new_pairs = vec![(0, 1), (2, 3), (4, 5)];
    for p in new_pairs {
        println!("{:?}", (p.0+1, p.1));
    }
    ...
    println!("{:?}", new_numbers); // should print [3, 6, 9, 12]
}
